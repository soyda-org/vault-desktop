...
def test_enroll_local_pin_bootstrap_requires_unlocked_vault(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert str(exc) == "Vault must be unlocked before enrolling PIN."


def test_unlock_session_vault_with_pin_round_trip(tmp_path: Path) -> None:
    service = make_service(tmp_path, user_id="user_1")
    login(service, identifier="alice")
    ...
    service.unlock_session_vault_with_pin(pin="1234")

    assert service.current_session_vault_master_key() == VALID_MASTER_KEY_B64


def test_unlock_session_vault_with_pin_rejects_wrong_pin(tmp_path: Path) -> None:
    service = make_service(tmp_path, user_id="user_1")
    login(service, identifier="alice")
    service.set_session_vault_master_key(VALID_MASTER_KEY_B64)
    ...
    try:
        service.unlock_session_vault_with_pin(pin="9999")
        assert False, "Expected ValueError"
    except ValueError as exc:
...
def test_unlock_session_vault_with_pin_rejects_other_account(tmp_path: Path) -> None:
    ...
    try:
        second.unlock_session_vault_with_pin(pin="1234")
        assert False, "Expected ValueError"
    except ValueError as exc:
        assert str(exc) == "Local PIN enrollment belongs to another account on this device."
